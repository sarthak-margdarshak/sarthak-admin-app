...
export default function ProductTranslatePage() {
  const { id: productId, targetLang } = useParams();
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const listRes = await appwriteDatabases.listDocuments(
          ...
          [Query.equal("productId", productId), Query.equal("lang", targetLang)]
        );

        if (!listRes || !listRes.documents || listRes.documents.length === 0) {
          setError(
            "Translated product not found. Please create translation first."
          );
        } else {
  ...
  const sourceLang = product.lang || "en";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!listRes || !listRes.documents || listRes.documents.length === 0) {
        enqueueSnackbar("Translated product not found.", { variant: "error" });
        setSaving(false);
        return;
      }
  ...
  const handleAITranslate = async () => {
    ...
    try {
      const response = await appwriteFunctions.createExecution(
        ...
        JSON.stringify({ productId, sourceLang: sourceLang, targetLang }),
        false,
        sarthakAPIPath.product.translate
      );
    ...
    <Box sx={{ maxWidth: 700, mx: "auto", my: 4 }}>
      <Paper sx={{ p: 3 }}>
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
          <Typography variant="h4">Translate Product</Typography>
          <Chip size="small" label={`#${product.productId || productId}`} />
        </Stack>
