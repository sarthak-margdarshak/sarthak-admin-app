...
export default function BookIndexFilter({ onChange }) {
  ...
  const [currValue, setCurrValue] = useState({id: null, label: "Click Edit to Select Index"});
  const [loading, setLoading] = useState(false);
  const [currInputValue, setCurrInputValue] = useState("");

  const onIndexValueChange = async () => {
    let orQueries = []
    if (currInputValue !== "" && currInputValue !== null && currInputValue !== undefined) {
      orQueries = [
    ...
    if(orQueries.length > 0) {queries.push(Query.or(orQueries))}
    setIsIndexLoading(true);
    try {
      const tem = (await appwriteDatabases.listDocuments(
  ...
  if (inEditMode) {
    return (
      <Fragment>
        ...
        {isIndexLoading ? <LoadingButton sx={{mt: 2}} variant='contained' loading={isIndexLoading}>Loading</LoadingButton> :
          ...
          {indexList.map((option, index) => (
            <MenuItem
              ...
              onClick={async (event) => {
                setLoading(true)
                setInEditMode(false)
                setCurrValue({id: option, label: event.target["textContent"]});
                setLoading(false)
                onChange(option)
              }}
            >
