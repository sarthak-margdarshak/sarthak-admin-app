...
export default function QuestionEditForm({ questionId }) {
  ...
  const [covers, setCovers] = useState({coverQuestion: null, coverOptions: [], coverAnswer: null});
  ...
  useEffect(() => {
    const update = async () => {
      ...
      setCovers({coverQuestion: question.coverQuestion, coverAnswer: question.coverAnswer, coverOptions: question.coverOptions});
  ...
  const saveQuestion = async () => {
    ...
    try {
      ...
      setQuestion(await appwriteDatabases.updateDocument(
        APPWRITE_API.databaseId,
        APPWRITE_API.collections.questions,
        questionId,
        {
  ...
  const uploadCover = async (coverFile, index) => {
    ...
    if (typeof coverFile !== "string") {
      if (currentCoverFile === null && currentCoverFile === "" && currentCoverFile === undefined) {
    ...
    <Fragment>
      ...
      <Box component="section" sx={{ p: 2, border: '1px dashed grey' }}>
        ...
        <Accordion>
          ...
          <AccordionDetails>
            <Box component="section" sx={{ p: 2, border: '1px dashed grey', '&:hover': {
              ...
              <Grid container spacing={1}>
                <Grid item xs={9}>
                  <ContentEditor
                    ...
                    onChange={(newVal) => setQuestion({...question, contentQuestion: newVal})}
                  />
                </Grid>
            ...
            {question?.contentOptions?.map((option, index) =>
              <Box component="section" sx={{
                ...
                <Grid container spacing={1} sx={{p: 1}}>
                  ...
                  <Grid item xs={7}>
                    <ContentEditor value={option || ""} onChange={(newVal) => {
                      question.contentOptions[index] = newVal;
                      setQuestion({...question, contentOptions: question.contentOptions})
                    }} />
                  </Grid>
                  ...
                  <Grid item xs={1}>
                    <Switch
                      ...
                      onChange={() => {
                        question.answerOptions[index] = !question.answerOptions[index];
                ...
                <Tooltip  title='Remove the option'>
                  <IconButton aria-label="settings" onClick={async () => {
                    ...
                    <Iconify icon="fluent:delete-48-filled" color="#d11a2a" width={20} />
                  </IconButton>
                </Tooltip>
              </Box>
            )}
              ...
              <Grid container spacing={1}>
                <Grid item xs={9}>
                  <ContentEditor
                    ...
                    onChange={(newVal) => setQuestion({...question, contentAnswer: newVal})}
                  />
                </Grid>
        ...
        <LoadingButton sx={{mt: 1}} variant='contained' loading={isSaving} onClick={saveQuestion}>Save</LoadingButton>
