...
import {appwriteDatabases, appwriteFunctions, timeAgo} from "auth/AppwriteContext";
...
export default function QuestionRowComponent({ questionId, defaultExpanded = true, showImages = true, showAnswer = true }) {
  ...
  const fetchData = async () => {
    try {
      ...
      if(question === undefined) {
        ...
        const isChanged = (await appwriteDatabases.getDocument(
    ...
    <React.Fragment>
      <Card sx={{ m: 1 }}>
        <CardHeader
          title={<Divider>
            <Chip
              label={question?.qnId + " (" + timeAgo.format(Date.parse(question?.lastSynced)) + ")"}
              color='primary'
          ...
          <Stack direction="row">
            <Tooltip title="Refresh">
              <IconButton aria-label="settings" onClick={async () => {
                ...
                <Iconify icon="solar:refresh-square-bold" color="#ff8164" width={35} />
              </IconButton>
            </Tooltip>
            {question?.published && <Tooltip title="Published">
              <Image
              ...
              {question?.contentOptions?.map((option, index) =>
                <Grid item sm={12} xs={12} md={6} lg={6} xl={6} key={index}>
                  <Alert
                    ...
                    severity={
                      question?.answerOptions[index] ? "success" : "info"
                    }
                    icon={<Iconify icon={"mdi:alphabet-"+String.fromCharCode(97 + index)+"-box"} />}
                    sx={{ m: 0.5 }}
                  >
                    <Stack direction="column">
                      <ReactKatex>{question?.contentOptions[index] || ""}</ReactKatex>
        ...
        <CardActions disableSpacing>
          {question?.published ?
            ...
            <Tooltip title="Publish">
              <IconButton disabled={question?.published} onClick={() => setOpenPublishDialog(true)}>
          ...
          {!question?.published && (
            <Tooltip title="Edit">
              <IconButton onClick={() => navigate(PATH_DASHBOARD.question.edit(questionId))}>
          ...
          {!defaultExpanded &&
            <Tooltip title={"View"}>
              <IconButton onClick={() => navigate(PATH_DASHBOARD.question.view(questionId))}>
                ...
                <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
                  <Item>
                    ...
                    >
                      ...
                      <Typography>{question.standard.standard + " 🢒 " + question.subject.subject + " 🢒 " + question.chapter.chapter + " 🢒 " + question.concept.concept}</Typography>
                    </Stack>
                  </Item>
                </Grid>
                ...
                <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
                  <Item>
                    ...
                    >
                      ...
                      <Typography>{question.published ? "Published" : "Draft"}</Typography>
                    </Stack>
                  </Item>
                </Grid>
                ...
                <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
                  <Item>
                    ...
                    >
                      ...
                      <SarthakUserDisplayUI userId={question.creator.$id} />
                    </Stack>
                  </Item>
                </Grid>
                  ...
                  <Item>
                    ...
                    >
                      ...
                      <Tooltip title={question?.$createdAt}>
                        <Typography>{timeAgo.format(Date.parse(question?.$createdAt))}</Typography>
                      </Tooltip>
                    </Stack>
                  </Item>
                </Grid>
                ...
                <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
                  <Item>
                    ...
                    >
                      ...
                      <SarthakUserDisplayUI userId={question.updater.$id} />
                    </Stack>
                  </Item>
                </Grid>
                  ...
                  <Item>
                    ...
                    >
                      ...
                      <Tooltip title={question?.$updatedAt}>
                        <Typography>{timeAgo.format(Date.parse(question?.$updatedAt))}</Typography>
                      </Tooltip>
                    </Stack>
                  </Item>
                </Grid>

                {question.published && <Fragment>
                  <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
                    <Item>
                      <Stack
                        direction="row"
                      ...
                      >
                        ...
                        <SarthakUserDisplayUI userId={question.approver.$id} />
                      </Stack>
                    </Item>
                  </Grid>
                    ...
                    <Item>
                      ...
                      >
                        ...
                        <Tooltip title={question?.approvedAt}>
                          <Typography>{timeAgo.format(Date.parse(question?.approvedAt))}</Typography>
                        </Tooltip>
                      </Stack>
                    </Item>
                  </Grid>
